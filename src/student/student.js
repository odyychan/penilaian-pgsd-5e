...
function normalizeMediaList(fieldOrMedia) {
    ...
    function renderOverviewGoogleStatus() {
      const card = document.getElementById("overviewGoogleAuthCard");
      ...
      if (email && (mode === 'ALL_EMAIL' || (mode === 'ULM_ONLY' && isUlm))) {
    ...
    function renderSingleClientFieldHtml(f) {
      ...
      if (f.type === 'CORE_IDENTITY' || f.type === 'CORE_IDENTITAS') {
        ...
        const effectiveAuthEmail = (activeUserAccountEmail || authSession?.email || authState?.user?.email || '').trim();
        if (effectiveAuthEmail && !activeUserAccountEmail) {
          activeUserAccountEmail = effectiveAuthEmail;
        }
        const emailVal = clientCustomFormAnswers[f.id + '_email'] || effectiveAuthEmail || '';
        let nameVal = clientCustomFormAnswers[f.id + '_nama'] || authSession?.nama || '';
        const nimVal = clientCustomFormAnswers[f.id + '_nim'] || activeUserAccountNim || authSession?.nim || '';
        const roleVal = clientCustomFormAnswers[f.id + '_peran'] || currentEvaluatorRole || authSession?.peran || 'Mahasiswa';
              ...
              ` : `
                <div class="space-y-1.5" id="emailFieldContainer">
                  <div class="flex items-center justify-between">
                    ...
                    ` : `
                      <span class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-amber-50 text-amber-800 border border-amber-200/80 text-[10px] font-semibold">
                        <span class="w-1.5 h-1.5 rounded-full bg-amber-500"></span>
                        <span>Akun Google Diperlukan</span>
                      </span>
                    `}
                  </div>
                  <div class="relative flex items-center">
                    <input 
                      ...
                      id="inputEmail" 
                      required 
                      ${(emailVal && effectiveAuthEmail) ? 'readonly' : ''}
                      value="${escapeHtml(emailVal)}"
                      placeholder="${effectiveAuthEmail ? 'Akun Google terverifikasi...' : 'nama@mhs.ulm.ac.id'}" 
                      class="w-full pl-3.5 ${effectiveAuthEmail ? 'pr-24' : 'pr-32'} py-2.5 rounded-xl border border-zinc-200 text-xs sm:text-sm ${(emailVal && effectiveAuthEmail) ? 'bg-zinc-50/80 cursor-default' : 'bg-white focus:border-zinc-900'} text-zinc-800 font-mono outline-none shadow-2xs"
                    ...
                    ${effectiveAuthEmail ? `
                      <button 
                        type="button" 
                        onclick="handleSwitchGoogleAccount()" 
                        class="absolute right-1.5 min-h-[32px] px-2.5 py-1 rounded-lg bg-white hover:bg-zinc-100 active:scale-95 text-zinc-700 border border-zinc-200 text-[11px] font-semibold transition cursor-pointer shadow-2xs flex items-center gap-1.5"
                        title="Ganti akun Google"
                      >
                    ...
                    ` : `
                      <button 
                        ...
                        class="absolute right-1.5 min-h-[32px] px-2.5 py-1 rounded-lg bg-zinc-900 hover:bg-zinc-800 active:scale-95 text-white text-[11px] font-semibold transition cursor-pointer shadow-2xs flex items-center gap-1.5"
                  ...
                  <p class="text-[10px] text-zinc-400">${effectiveAuthEmail ? 'Email diverifikasi melalui akun Google terdaftar.' : 'Masuk dengan Google agar email terkunci otomatis dan draf tersimpan.'}</p>
                </div>
              `}
            </div>
          </div>
    ...
    function getActiveSessionRatingStatus(targetNim, targetEmail) {
      const activeSesi = (appConfig["Sesi_Minggu_Aktif"] || appConfig["Sesi_Aktif"] || currentFormMeta?.sesiAktif || "").trim();
      const isFilterAllSesi = !activeSesi || /^(semua|all|semua sesi|semua minggu)$/i.test(activeSesi);

      let groupsInActiveSesi = [];
      if (groupsData && groupsData.length > 0) {
        groupsInActiveSesi = groupsData.filter(g => {
          if (!isFilterAllSesi && g.sesi && g.sesi.trim().toLowerCase() !== activeSesi.toLowerCase()) return false;
          return true;
        });
      }

      const activeNim = (targetNim || document.getElementById("inputNim")?.value || activeUserAccountNim || "").replace(/\s+/g, "").trim().toLowerCase();
      const activeEmail = (targetEmail || activeUserAccountEmail || getCurrentAuthSession()?.email || "").trim().toLowerCase();

      const alreadyFilledGroups = [];
      if (currentRekapData) {
        if (activeNim && currentRekapData.nimToKelompokMap && currentRekapData.nimToKelompokMap[activeNim]) {
          currentRekapData.nimToKelompokMap[activeNim].forEach(g => {
            if (!alreadyFilledGroups.some(f => f.toLowerCase() === g.toLowerCase())) alreadyFilledGroups.push(g);
          });
        }
        if (activeEmail && currentRekapData.emailToKelompokMap && currentRekapData.emailToKelompokMap[activeEmail]) {
          currentRekapData.emailToKelompokMap[activeEmail].forEach(g => {
            if (!alreadyFilledGroups.some(f => f.toLowerCase() === g.toLowerCase())) alreadyFilledGroups.push(g);
          });
        }
      }
      ...
      const isSessionFullyCompleted = groupsInActiveSesi.length > 0 && unfilledGroups.length === 0;
      const hasPartialProgress = groupsInActiveSesi.length > 0 && alreadyFilledGroups.length > 0 && unfilledGroups.length > 0;
    ...
    function evaluateFormScheduleStatus() {
      ...
      if (!isBlocked && (appConfig["Kunci_Respons_Ganda"] === true || appConfig["Kunci_Respons_Ganda"] === 'true')) {
        ...
        if (!isIdentified) {
          ...
          if (isSubmittedInStorage) {
            const status = getActiveSessionRatingStatus(activeNim, activeEmail);

            if (status.isSessionFullyCompleted) {
              // Semua kelompok di sesi aktif sudah dinilai → blokir global
              isBlocked = true;
              blockReason = "ALREADY_SUBMITTED";
              const sesiLabel = status.activeSesi || "sesi aktif";
              blockTitle = "Semua Kelompok Telah Dinilai";
              blockDesc = `Anda telah menyelesaikan penilaian untuk seluruh ${status.groupsInActiveSesi.length} kelompok presentator pada <strong>${escapeHtml(sesiLabel)}</strong>. Terima kasih atas partisipasi Anda!<br><span class="inline-block mt-2 font-mono font-bold text-emerald-700 bg-emerald-50 px-2.5 py-1 rounded-lg border border-emerald-200">✓ ${status.alreadyFilledGroups.length} / ${status.groupsInActiveSesi.length} Kelompok Selesai Dinilai</span>`;
            } else if (status.hasPartialProgress) {
              ...
              if (progressBanner) {
                ...
                if (descEl) {
                  ...
                  descEl.innerHTML = `Anda sudah menilai <strong>${status.alreadyFilledGroups.length}</strong> dari <strong>${status.groupsInActiveSesi.length}</strong> kelompok presentator di sesi ini. Masih ada <strong>${status.unfilledGroups.length}</strong> kelompok lagi yang perlu dinilai.<br><span class="inline-block mt-1.5 font-mono text-[10px] text-emerald-700 bg-emerald-50 px-2 py-0.5 rounded border border-emerald-200">✓ Sudah dinilai: ${status.alreadyFilledGroups.map(g => escapeHtml(g)).join(", ")}</span>`;
                }
              }
              // Jangan blokir — biarkan mahasiswa lanjut menilai kelompok berikutnya
            } else if (status.groupsInActiveSesi.length === 0 && isSubmittedInStorage) {
    ...
    function renderAccountBar(identity) {
      ...
      const miniGoogleBadge = document.getElementById("accountGoogleMiniBadge");
      if (miniGoogleBadge) miniGoogleBadge.classList.remove("hidden");
    ...
    function showGoogleAuthGate() {
      ...
      if (courseCodeEl) {
        ...
        const kls = (currentFormMeta && currentFormMeta.kelas) || appConfig["Kelas"] || "";
        courseCodeEl.textContent = `${matkul}${kls ? ' • Kelas ' + kls : ''}`;
      }
      if (formTitleEl) {
        formTitleEl.textContent = (currentFormMeta && currentFormMeta.judulForm) || appConfig["Judul_Form"] || "Penilaian Presentasi Mahasiswa";
      }
    ...
    function showDomainMismatch(profile) {
      ...
      if (emailEl) emailEl.textContent = profile?.email || 'Email Tidak Diketahui';
    ...
    async function startAssessmentForm() {
      ...
      if (mode === 'NO_EMAIL') {
        if (appConfig && (appConfig["Kunci_Respons_Ganda"] === true || appConfig["Kunci_Respons_Ganda"] === 'true')) {
          const ratingStatus = getActiveSessionRatingStatus();
          if (ratingStatus.isSessionFullyCompleted) {
            showToast("Anda telah menyelesaikan penilaian untuk seluruh kelompok pada sesi ini.", "info");
            goToInfoOverview();
            if (typeof evaluateFormScheduleStatus === 'function') evaluateFormScheduleStatus();
            return;
          }
    ...
    function continueAssessmentWithAuthenticatedUser(user, mode) {
      ...
      // 🛡️ GATEKEEPER SESI: Periksa apakah seluruh kelompok di sesi aktif sudah dinilai
      if (appConfig && (appConfig["Kunci_Respons_Ganda"] === true || appConfig["Kunci_Respons_Ganda"] === 'true')) {
        const ratingStatus = getActiveSessionRatingStatus(identity.nim, identity.email);
        if (ratingStatus.isSessionFullyCompleted) {
          showToast("Anda telah menyelesaikan penilaian untuk seluruh kelompok pada sesi ini.", "info");
          goToInfoOverview();
          if (typeof evaluateFormScheduleStatus === 'function') {
    ...
    function openAssessmentForm(pushState = false) {
      // 🛡️ HARD DEFENSE: Pastikan sesi aktif belum dinilai penuh untuk akun aktif
      if (appConfig && (appConfig["Kunci_Respons_Ganda"] === true || appConfig["Kunci_Respons_Ganda"] === 'true')) {
        const ratingStatus = getActiveSessionRatingStatus();
        if (ratingStatus.isSessionFullyCompleted) {
          goToInfoOverview();
    ...
    async function executeConfirmedFinalSubmit() {
      ...
      try {
        ...
        if (sbSuccess) {
          ...
          // ⚡ SINKRONISASI OPTIMIS INSTAN KE MEMORI REKAPITULASI (< 1 ms)
          if (!currentRekapData) {
            currentRekapData = { summary: [], nimToKelompokMap: {}, emailToKelompokMap: {}, rawResponses: [] };
          }
          ...
          if (cleanNim) {
            ...
            if (!currentRekapData.nimToKelompokMap[cleanNim].some(g => g.toLowerCase() === grpName.toLowerCase())) {
              currentRekapData.nimToKelompokMap[cleanNim].push(grpName);
            }
          }
          if (cleanEmail) {
            if (!currentRekapData.emailToKelompokMap[cleanEmail]) currentRekapData.emailToKelompokMap[cleanEmail] = [];
    ...
    function renderPrintPreviewContent() {
      ...
      // Read custom Print_Config if configured
      const printCfg = appConfig.Print_Config || {};
      const order = printCfg.blocksOrder || ['kop', 'identity', 'rekap', 'reviews', 'signatures', 'footer'];
      ...
      order.forEach(blockKey => {
        const block = printCfg[blockKey] || {};
        if (printCfg[blockKey] && block.enabled === false) return;

        // 1. KOP SURAT
        if (blockKey === 'kop') {
          const kementerian = block.kementerian || appConfig["Kementerian"] || "KEMENTERIAN PENDIDIKAN TINGGI, SAINS, DAN TEKNOLOGI";
          const universitas = block.universitas || appConfig["Universitas"] || "UNIVERSITAS LAMBUNG MANGKURAT";
          const fakultas = block.fakultas || appConfig["Fakultas"] || "FAKULTAS KEGURUAN DAN ILMU PENDIDIKAN";
          const rawJurusan = (block.programStudi || appConfig["Program_Studi"] || appConfig["Jurusan"] || (currentFormMeta && currentFormMeta.jurusan) || "PGSD").trim();
          ...
          const alamatInstansi = block.alamat || appConfig["Alamat_Instansi"] || "Jl. Brigjen H. Hasan Basry, Kayu Tangi, Banjarmasin, Kalimantan Selatan 70123 • Laman: fkip.ulm.ac.id";
          const logoUrl = block.logoUrl || appConfig["Logo_Url"] || "assets/logo-ulm.png";
        ...
        if (blockKey === 'identity') {
          ...
          let reportTitle = block.reportTitle || "LAPORAN REKAPITULASI HASIL PENILAIAN PRESENTASI";
          if (!block.reportTitle && formTitle && !formTitle.toLowerCase().includes("penilaian presentasi") && formTitle.length < 50) {
        ...
        if (blockKey === 'rekap') {
          const minScore = (appConfig && appConfig["Nilai_Kelompok_Min"] !== undefined) ? appConfig["Nilai_Kelompok_Min"] : 0;
          const maxScore = (appConfig && appConfig["Nilai_Kelompok_Max"] !== undefined) ? appConfig["Nilai_Kelompok_Max"] : 100;
          ...
          mainContentHtml += `
            <div class="space-y-1 print-avoid-break" style="margin-top: 14px; padding-top: 2px; margin-bottom: 8px;">
              <div style="display: flex; align-items: center; justify-content: space-between; margin-bottom: 3px;">
                ...
                <span style="font-size: 10px; font-family: monospace; color: #4b5563;">${escapeHtml(block.scaleText || `Skala Penilaian: ${minScore} - ${maxScore}`)}</span>
              </div>
        ...
        if (blockKey === 'reviews' && includeReviews && summaryList.length > 0) {
          mainContentHtml += `
        ...
        // 5. LEMBAR PENGESAHAN (MULTI-SIGNATORY)
        if (blockKey === 'signatures') {
          const sigs = block.signatories || [];
          const kotaStr = block.kota || appConfig["Kota_Instansi"] || "Banjarmasin";
          ...
          if (sigs.length > 0) {
            ...
            // Fallback single signatory
            const dosen = appConfig["Dosen_Pengampu"] || (currentFormMeta && currentFormMeta.dosen) || "";
            const nip = appConfig["NIP_Dosen"] || (dosen.toLowerCase().includes("ririanti") ? "19830514 200812 2 003" : "");
            if (dosen) {
              mainContentHtml += `
                <div class="print-signature print-avoid-break" style="page-break-inside: avoid; break-inside: avoid; margin-top: 16px; display: flex; justify-content: flex-end; font-family: 'Times New Roman', Times, serif;">
                  <div style="text-align: center; min-width: 260px; max-width: 380px; width: fit-content;">
                    <p style="margin: 0; font-size: 11.5px; color: #000000;">${escapeHtml(kotaStr)}, ${printDateStr}</p>
                    <p style="margin: 1.5px 0 0 0; font-size: 11.5px; font-weight: 700; color: #000000;">Dosen Pengampu Mata Kuliah,</p>
                    ...
                    <div style="margin-top: 1px; padding: 0 6px 1.5px 6px; border-bottom: 1.5px solid #000000; display: inline-block; min-width: 220px; max-width: 100%;">
                      <span style="font-size: 12px; font-weight: 800; color: #000000; white-space: nowrap; letter-spacing: 0.01em;">${escapeHtml(dosen)}</span>
                    </div>
                    ${nip ? `<p style="margin: 2px 0 0 0; font-size: 11px; color: #000000; font-weight: 600;">NIP. ${escapeHtml(nip)}</p>` : ''}
                  </div>
                </div>
              `;
            }
      ...
      // 6. FOOTER DOKUMEN
      const footerBlock = printCfg.footer || {};
      let footerHtml = '';
      if (includeFooter && (footerBlock.enabled !== false) && order.includes('footer')) {
        const rawProdi = appConfig["Program_Studi"] || appConfig["Jurusan"] || (currentFormMeta && currentFormMeta.jurusan) || "PGSD";
        const univ = printCfg.kop?.universitas || appConfig["Universitas"] || "Universitas Lambung Mangkurat";
